...
def main(args):
    ...
    if(args.rank_models == "True"):
        ...
        ax.plot(np.array(hls), np.array(maes_0),   marker=next(marker), label="Dropout = 0")
        ax.plot(np.array(hls), np.array(maes_025), marker=next(marker), label="Dropout = 0.25")
        ax.plot(np.array(hls), np.array(maes_050), marker=next(marker), label="Dropout = 0.5")
        ax.legend()
        ax.xaxis.set_ticks(hls)
        ax.set_title("MAE x HL por dropout")
        ...
        ax.plot(np.array(hls), np.array(maes_24), marker=next(marker), label="Nx = 24")
        ax.plot(np.array(hls), np.array(maes_48), marker=next(marker), label="Nx = 48")
        ax.legend()
        ax.xaxis.set_ticks(hls)
        ax.set_title("MAE x HL por Janelamento traseiro (Nx)")
        ax.set_ylabel("MAE")
        ax.set_xlabel("HL")
